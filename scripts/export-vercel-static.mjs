...
const productSlugs = [...productsSource.matchAll(/\bid:\s*"[^"]+"\s*,\s*slug:\s*"([^"]+)"/g)].map((match) => match[1]);
const routes = ["/", "/catalogo", "/pesquisa", "/politicas", "/rastrear", "/conta", "/carrinho", "/checkout", ...productSlugs.map((slug) => `/produto/${slug}`)];
...
function makeStatic(html) {
  return html
    ...
    .replace(/\s+srcSet="[^"]*"/g, "")
    .replace(/src="\/_next\/image\?url=([^&"]+)(?:&amp;|&)w=[^"]+"/g, (_, encodedUrl) => `src="${decodeURIComponent(encodedUrl)}"`)
...
if (cssFiles.length !== 1) throw new Error(`Esperado exatamente um stylesheet compilado; encontrados: ${cssFiles.join(", ")}`);
